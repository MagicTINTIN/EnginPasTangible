...
vec2 SDF_Global(vec3 p){
	return vec2(SDF_Box_Frame(infinity(p,vec3(3.0)),vec3(1.),0.07),420);
}
...
vec3 Get_Color(vec3 origin,vec3 dir){
	...
	vec3 sunDir=normalize(sunPos-impact.xyz);
	vec3 normale=grad(impact.xyz);
	//vec4 ombre = Get_Impact(impact.xyz+0.02*normale,sunDir);
	//float f=ombre.w<0.?1.:.5;
	vec3 couleur = vec3(1.);
    
    return couleur*clamp(dot(sunDir,normale),0.,1.);
}
