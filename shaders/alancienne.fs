...
vec2 SDF_Global(vec3 p){
    vec2 res = vec2(SDF_Box_Frame(p,vec3(1.),.1),420.); //deuxième value:couleur : valeur de hue entre 0 et 360
	// Par exemple
	// 000 : rouge
	// 060 : jaune
	// 120 : vert
    ...
    res=opu(res,vec2(vec2(SDF_Box(p+vec3(0,2,0),vec3(5,.2,5)),420)));
	return res;
}
...
vec3 Get_Color(vec3 origin,vec3 dir){
	...
	return couleur*clamp(dot(sunPos,normale),0.,1.);
}
